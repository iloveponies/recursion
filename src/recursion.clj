...
(defn rising? [a-seq]
  (cond
    (empty? a-seq) true
    (== 1 (count a-seq)) true
    (<= (first a-seq) (second a-seq)) (rising? (rest a-seq))
    :else false))

(defn decreasing? [a-seq]
  (rising? (reverse a-seq)))

(defn monotonic? [a-seq]
  (or (rising? a-seq) (decreasing? a-seq)))
...
(defn permutation-helper [a-seq result i j]
  (let [length (dec (count a-seq))]
    (if (== length i)
      ...
      (let [swapped (seq (swap a-seq i j))
            ...
            new-i (inc i)]
        (if (== j length)
          (permutation-helper swapped new-result new-i 0)
          (let [original-swap (permutation-helper a-seq new-result i (inc j))
                new-swap (permutation-helper swapped new-result i (inc j))]
            (concat (seq new-result) original-swap new-swap)))))))

(defn permutations [a-seq]
  (if (empty? a-seq)
    '(())
    (seq (set (permutation-helper a-seq '() 0 0)))))
  ...
  (let [length (count a-seq)]
    (cond
      ...
      :else (let [subset (powerset (rest a-seq))
                  concat-subset (union subset (first a-seq))]
