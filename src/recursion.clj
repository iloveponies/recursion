...
(defn seq= [a-seq b-seq]
  (cond
    ...
    (or (and (empty? a-seq) (not-empty b-seq)) (and (not-empty a-seq) (empty? b-seq))) false
...
(defn halve [a-seq]
  (let [first-half-count (int (/ (count a-seq) 2))]
    (vector (my-take first-half-count a-seq) (my-drop first-half-count a-seq))))
...
(defn split-into-monotonics [a-seq]
  (if (empty? a-seq)
    ()
    (let [longest (longest-sequence (take-while
                                      #(or (= (merge-sort %) %) (= (reverse (merge-sort %)) %))
                                      (rest (reverse (inits a-seq)))))]
