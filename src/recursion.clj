...
(defn halve [a-seq]
  (let [ split (int (/ (count a-seq) 2)) ]
    (seq (vector (take split a-seq)(drop split a-seq)))))
...
(defn seq-merge [a-seq b-seq]
  (let [ helper (fn [a-seq b-seq acc]
                  (cond
                    ...
                    :else (let [seq1 (first a-seq) seq2 (first b-seq)]
                            (if (< seq1 seq2)
                              (recur (rest a-seq) b-seq (concat acc [seq1]))
                              (recur a-seq (rest b-seq) (concat acc [seq2]))))))](helper a-seq b-seq '())))
...
(defn powerset [a-set]
  (if (empty? a-set)(hash-set (hash-set))
