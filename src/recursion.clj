...
(defn merge-sort [a-seq]
  (if (< (count a-seq) 2)
    a-seq
    (let [new-seq (halve a-seq)]
	  (seq-merge (merge-sort (first new-seq)) (merge-sort (second new-seq))))))
  ...
  (if (empty? a-seq)
    ...
    (let [monotonic? (fn [x]
                       (if (empty? x)
                         true
                         (or (apply < x) (apply > x))))
          new-elem (last (take-while monotonic? (inits a-seq)))]
      (cons new-elem (split-into-monotonics (drop (count new-elem) a-seq))))))

(defn permutations [a-set]
  (if (empty? a-set)
    [[]]
    (let [all-permutations (fn[x] (map (fn [y] (cons (first x) y))
                                       (permutations (rest x))))]
...
(defn powerset [a-set]
  (if (empty? a-set)
    #{#{}}
    (let [rest-powerset (powerset (rest a-set))]
      (clojure.set/union
        rest-powerset
        (set (map (fn [x] (clojure.set/union #{(first a-set)} x))
                  rest-powerset))))))
