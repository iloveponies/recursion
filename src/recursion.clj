...
(defn halve [a-seq]
  (let
    [midCount (int (/ (count a-seq) 2))]
    (cons (my-take midCount a-seq) [(my-drop midCount a-seq)])
  )
)
...
(defn permutations [a-set]
  (if
    (seq (rest a-set))
    (apply concat (for [x a-set] (map #(cons x %) (permutations (remove #{x} a-set)))))
    (list a-set)
  )
)
...
(defn powerset [a-set]
  (if
    ...
    (set (apply concat
      (for
        [x a-set]
        (concat [#{}] [#{x}] (map set (map #(cons x %) (powerset (remove #{x} a-set)))))
      )
    ))
    (list a-set)
  )
