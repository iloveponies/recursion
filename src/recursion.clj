...
(defn seq-merge [a-seq b-seq]
   (let [m (fn [a-seq b-seq acc]
      (cond
        (empty? a-seq)(concat acc b-seq)
        (empty? b-seq)(concat acc a-seq)
      :else
        (let [s1 (first a-seq) s2 (first b-seq)]
          (if (< s1 s2)
          (recur (rest a-seq) b-seq (concat acc [s1]))
                          (recur a-seq (rest b-seq) (concat acc [s2]))))))]
        (m a-seq b-seq '())))
...
(defn split-into-monotonics [a-seq]
     ...
     split-help (fn [prefix suffix](if (monotonic? prefix)
       (list prefix suffix)
