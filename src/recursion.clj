...
(defn my-take [n coll]
  (cond
    (empty? coll) (lazy-seq nil)
    ((complement pos?) n) (lazy-seq nil)
    :else (cons (first coll) (my-take (dec n) (rest coll)))))

(defn my-drop [n coll]
  (cond
    (empty? coll) (lazy-seq nil)
    ((complement pos?) n) coll
...
(defn split-into-monotonics [a-seq]
  (if (empty? a-seq)
    a-seq
    (let [first-monotonic (last (take-while (fn [x] (or (empty? x) (apply <= x) (apply >= x))) (inits a-seq)))
          remaining (drop (count first-monotonic) a-seq)]
      (cons first-monotonic (split-into-monotonics remaining)))))
...
(defn powerset [a-set]
  (if (empty? a-set)
    ...
    (let [next-elem (first a-set)
      ...
      (clojure.set/union powerset-rest (map (fn [ps] (conj ps next-elem)) powerset-rest)))))
