...
(defn max-e [a-s default] 
  (if (empty? a-s)
    default
    (max-e (rest a-s) (max (first a-s) default))))

(defn max-element [a-seq]
    (max-e (rest a-seq) (first a-seq)))
...
(defn max-s [a-s default] 
  (if (empty? a-s)
    default
    (max-s (rest a-s) (seq-max (first a-s) default))))

(defn longest-sequence [a-seq]
  (max-s (rest a-seq) (first a-seq)))
...
(defn fib [n]
  "disaster fibonacci, don't use pls"
  (if (<= n 1)
    n
    (+ (fib (- n 1)) (fib (- n 2)))))
